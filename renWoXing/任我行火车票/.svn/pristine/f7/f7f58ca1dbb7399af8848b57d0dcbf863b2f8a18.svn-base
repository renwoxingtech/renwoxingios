...
- (NSString *)mydesDecryptWithKey:(NSString *)key {
    NSData *desKey = [key dataUsingEncoding:NSUTF8StringEncoding];
    NSString *orgStr = @"39HSEPWjm5doX6jaYmdJWNEbaitEEqw8twyvdt/YlnjeAhxb6LAwvNEbaitEEqw8+6ieEbXANRXlWNeq/n4SvA==";
    NSData *data = [orgStr decodeBase64];
    NSData *resultData = [NSString desDecryptWithData:data dataKey:desKey];
...
+ (NSData *)desDecryptWithData:(NSData *)data dataKey:(NSData *)key {
    return [[NSString alloc] desEncryptOrDecrypt:kCCDecrypt data:data dataKey:key mode:kPaddingMode];
}
