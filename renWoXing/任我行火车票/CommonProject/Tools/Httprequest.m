...
-(void)postObjectByParameters:(id)parameters andUrl:(NSString *)urlString showLoading:(BOOL)showLoading showMsg:(BOOL)showMsg isFullUrk:(BOOL)isFull andComplain:(returnObject)complain andError:(returnError)Error{
    ...
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
    self.manager = [Httprequest shareSessionManger];
    self.manager.requestSerializer.timeoutInterval = 35;
    self.manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/json", @"text/javascript", @"text/html",@"text/plain",@"application/javascript",@"text/html; charset=utf-8", nil];//
//    self.manager.requestSerializer = [AFHTTPRequestSerializer serializer];
//    self.manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    

    NSLog(@"uuSTRing%@ ,parameters\n%@",uuSTRing,parameters);
...
//    }
    ...
    [self.manager POST:uuSTRing parameters:dic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (!([mmsg isEqual:[NSNull null]] ||[mmsg isEqualToString:@"<null>"])) {
            if (showMsg && ![mmsg isEqualToString:@"成功"] && ![mmsg isEqualToString:@"正常获得结果"]) {


            }
        }else{
            [MBProgressHUD showHudWithString:@"未知信息" model:MBProgressHUDModeCustomView];
        }
        if ([dd isKindOfClass:[NSDictionary class]]) {
            if ([dd[@"code"] integerValue] == 10004) {
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [MBProgressHUD showHudWithString:@"需要重新登录" model:MBProgressHUDModeCustomView];
