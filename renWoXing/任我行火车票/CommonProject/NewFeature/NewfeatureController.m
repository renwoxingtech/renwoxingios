...
{
    if (self = [super init]) {
//        NSArray *coverNames = @[@"img_index_01txt", @"img_index_02txt", @"img_index_03txt"];
//        NSArray *bgNames = @[@"img_index_01bg", @"img_index_02bg", @"img_index_03bg"];
        
        NSArray *coverNames;
//        if (isIPhone5){
//            coverNames = @[@"img5_index01bg", @"img5_index02bg"];
//        }else if (isIPhone6){
//            coverNames = @[@"img6_index01bg", @"img6_index02bg"];
//        }else if (isIPhone6p){
//            coverNames = @[@"img6p_index01bg", @"img6p_index02bg"];
//        }else{
            coverNames = @[@"image1", @"image2",@"image3"];
//        }
        NSArray *bgNames = nil;
...
- (void)viewDidLoad {
    ...
    if (!self.enterButton) {
        ...
        [self.enterButton setTitleColor:[UIColor clearColor] forState:UIControlStateNormal];
...
- (void)enter:(id)object
{
    ...
    // 切换窗口的根控制器
    ((AppDelegate *)[UIApplication sharedApplication].delegate).tabBarController = [[WXTabbarViewController alloc]init];
    ((AppDelegate *)[UIApplication sharedApplication].delegate).window.rootViewController = ((AppDelegate *)[UIApplication sharedApplication].delegate).tabBarController = [[WXTabbarViewController alloc]init];
    
//    if (!IsStrEmpty([Config currentConfig].loginName)) {
        /*
         如果登陆APP则登陆环信，登陆环信，设置为自动登录
         */
//        [[AutoLogin shareInstance] startAutoLogin];
//
//    }
